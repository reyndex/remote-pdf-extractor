...
from extractor.enums import ExtractionErrorCode, UploadRequestField
...
def _json_response(payload: ExtractionResponse):
    response = jsonify(payload)
    response.status_code = http_status_code_for_response(payload)
...
def extract_document(request):
    if request.method != "POST":
        return _json_response(error_payload(POST_INSTRUCTIONS))
    ...
    try:
        ...
        return _json_response(error_payload(str(exc)))

    if file_url is None:
        return _json_response(
            error_payload(MISSING_FILE_MESSAGE, code=ExtractionErrorCode.MISSING_FILE)
        )

    return _json_response(_process_file_url(file_url))
